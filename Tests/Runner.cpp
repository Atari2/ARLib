...
TEST(ARLibTests, UniqueStr) {
    ...
    String s3 = s2->concat("help");
    EXPECT_EQ(s3, "otherhelp"_s);
}
...
TEST(ARLibTests, IteratorChainingTest) {
    ...
    auto a = vec.iter().all([](int v) { return v > 0; });
    EXPECT_EQ(a, true);
    auto b = vec.iter().any([](int v) { return v > 3; });
    EXPECT_EQ(b, true);
    auto c = vec.iter().any([](int v) { return v > 5; });
    EXPECT_EQ(c, false);
    auto d = vec.iter().max();
    EXPECT_EQ(*d, 5);
    auto e = vec.iter().min();
    ...
    auto f = vec.iter().sum();
    EXPECT_EQ(f, 15);
    auto g = vec.iter().filter([](int v) { return v % 2 == 0; }).map([](int v) { return v * 2; }).sum();
    EXPECT_EQ(g, 12);
    auto h = vec.iter().find(3);
    EXPECT_TRUE(h.has_value());
    EXPECT_EQ(*h, 3);
    auto i = vec.iter().find(6);
    EXPECT_FALSE(i.has_value());
    auto j = vec.iter().find_if([](int v) { return v == 3; });
    EXPECT_TRUE(j.has_value());
    EXPECT_EQ(*j, 3);
    auto k = vec.iter().find_if([](int v) { return v == 6; });
    EXPECT_FALSE(k.has_value());
    auto l = vec.iter().filter([](int v) { return v % 2 == 0; }).map([](int v) { return IntToStr(v * 2); }).find("8"_s);
