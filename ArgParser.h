	...
	public:
		...
		public:
			...
			bool operator==(const String& name) {
				return m_long_name == name || m_short_name == name;
			}
			String as_string() {
				return m_value.value_or("");
			}
			int as_int() {
				return StrToInt(as_string());
			}
			float as_float() {
				return StrToFloat(as_string());
			}
			bool as_bool() {
				return StrToBool(as_string());
			}
		};
	private:
