...
class SortedVector {
    ...
    constexpr static inline auto npos = static_cast<size_t>(-1);
    Ordering m_ordering{};
    T* m_storage      = nullptr;
    size_t m_size     = 0;
    size_t m_capacity = 0;
    ...
    void insert_single_element_(T&& element) {
        ...
        if constexpr (IsTriviallyCopiableV<T>) {
            ...
            for (size_t i = m_size; i > insert_index; i--) { m_storage[i] = move(m_storage[i - 1]); }
        }
    ...
    void for_each(Functor func) {
        for (const auto& item : *this) func(item);
    }
