...
namespace ARLib {
    ...
    class LinkedSet {
        struct SetEntry {
            ...
            SetEntry& operator=(const SetEntry& other) requires CopyAssignable<T> {
            ...
            SetEntry& operator=(SetEntry&& other) noexcept requires MoveAssignable<T> {
        ...
        const T& peek() const requires DefaultConstructible<T> {
            if (m_size == 0) return {};
            return m_head->entry();
        }
