...
class FixedMatrix2D {
    ...
    constexpr static bool ValidToInline = traits::ValidToInline;
    constexpr static bool Square        = traits::Square;
    ...
    static int rank_internal(typename traits::ConstMatRefType og_matrix) {
        ...
        if constexpr (!ValidToInline) {
            for (size_t i = 0; i < N; i++) { deallocate<T, DeallocType::Multiple>(matrix[i]); }
            deallocate<T*, DeallocType::Multiple>(matrix);
        }
        return rank;
    }
    static double det_internal(typename traits::ConstMatRefType og_matrix)
    requires Square
    {
        typename traits::MatType matrix{};
    ...
    requires(!ValidToInline)
    {
        for (size_t i = 0; i < N; i++) { deallocate<T, DeallocType::Multiple>(m_matrix[i]); }
    ...
    FixedMatrix2D(const T (&matrix)[N][M])
    requires ValidToInline
    {
        for (size_t i = 0; i < N; i++) { ConditionalBitCopy(m_matrix[i], matrix[i], M); }
    }
    /* COPY AND MOVE CTORS AND ASSIGNMENT OPS*/

    FixedMatrix2D(const FixedMatrix2D& other) {
        if constexpr (!ValidToInline) { allocate_memory(); }
        for (size_t i = 0; i < N; i++) { ConditionalBitCopy(m_matrix[i], other.m_matrix[i], M); }
    }
    ...
    auto columns_begin(size_t begin_idx = 0) { return FixedMatrixIterator<N, M, false, false>{ m_matrix, begin_idx }; }
    auto columns_end() { return FixedMatrixIterator<N, M, false, false>{ m_matrix, M }; }
    auto rows_begin(size_t begin_idx = 0) const { return FixedMatrixIterator<N, M, true, true>{ m_matrix, begin_idx }; }
...
#define DEFINE_OP(op, in_op)                                                                                           \
    ...
    FixedMatrix2D inv() const
    requires Square
    {
        FixedMatrix2D<N, M * 2> mat_glued{};
        for (size_t i = 0; i < N; i++) {
            for (size_t j = 0; j < M; j++) { mat_glued[{ i, j }] = m_matrix[i][j]; }
        }
        for (size_t j = M; j < M * 2; j++) { mat_glued[{ j - M, j }] = T{ 1 }; }
        mat_glued.reduce();
    ...
    FixedMatrix2D<M, N> transpose() const {
        ...
        for (size_t i = 0; i < N; i++) {
            for (size_t j = 0; j < M; j++) { mat[{ j, i }] = m_matrix[i][j]; }
        }
        return mat;
    }
...
FixedMatrix2D<N1, 1> operator*(const FixedMatrix2D<N1, M>& first, const FixedColumnArray<M, N2, IsConst>& column) {
    ...
    for (size_t row = 0; row < N1; row++) {
        double intermediate{ 0 };
        for (size_t i = 0; i < M; i++) { intermediate += first[{ row, i }] * column[i]; }
...
FixedMatrix2D<1, N2> operator*(const FixedRowArray<N1, M, IsConst>& row, const FixedMatrix2D<M, N2>& second) {
    ...
    for (size_t col = 0; col < N2; col++) {
        double intermediate{ 0 };
        for (size_t i = 0; i < M; i++) { intermediate += row[i] * second[{ i, col }]; }
...
FixedMatrix2D<N, M> operator+(const FixedMatrix2D<N, M>& first, const FixedMatrix2D<N, M>& second) {
    ...
    for (size_t i = 0; i < N; i++) {
        for (size_t j = 0; j < M; j++) { mat[{ i, j }] = first[{ i, j }] + second[{ i, j }]; }
    }
    return mat;
}
template <size_t N, size_t M>
