...
class FunctionBase {
    public:
    constexpr static size_t m_max_size  = sizeof(fntraits::NocopyTypes);
    constexpr static size_t m_max_align = alignof(fntraits::NocopyTypes);
    template <typename Functor>
    class BaseManager {
        protected:
        static const bool m_stored_locally =
        (fntraits::IsLocationInvariant<Functor>::value && sizeof(Functor) <= m_max_size && alignof(Functor) <= m_max_align && (m_max_align % alignof(Functor) == 0));
...
class Function<Res(Args...)> : public fntraits::MbUOrBFn<Res, Args...>, private FunctionBase {
    ...
    template <
    typename Functor, typename = Requires<Not<IsSame<Functor, Function>>, void>,
