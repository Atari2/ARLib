...
static_assert(
alignof(mutex_internal_imp_t) <= Mutex_internal_imp_alignment, "incorrect alignment of mutex implementation"
);

static_assert(sizeof(cnd_internal_imp_t) <= Cond_internal_imp_size, "incorrect condition variable implementation");
static_assert(
alignof(cnd_internal_imp_t) <= Cond_internal_imp_alignment, "incorrect condition variable implementation"
);
...
ThreadState __cdecl thread_join(ThreadHandle thread, int* ptr) {
    unsigned long res = 0;
    if (WaitForSingleObjectEx(thread._Hnd, INFINITE, FALSE) == WAIT_FAILED || GetExitCodeThread(thread._Hnd, &res) == 0) {
    ...
    if ((to_enum<MutexType>((mutex->type & ~from_enum(MutexType::Recursive))) & MutexType::Plain) != MutexType::None) {
        ...
        if (target == nullptr) {
            ...
            while (now.sec < target->sec || (now.sec == target->sec && now.nsec < target->nsec)) {
                if (mutex->thread_id == static_cast<long>(GetCurrentThreadId()) || mutex->_get_cs()->try_lock_for(time_diff_to_millis2(target, &now))) {
                    res = WAIT_OBJECT_0;
                    break;
                } else {
                    res = WAIT_TIMEOUT;
...
ThreadState __cdecl mutex_lock(MutexHandle mutex) {
    return mtx_do_lock(mutex, nullptr);
}
ThreadState __cdecl mutex_trylock(MutexHandle mutex) {
    XTime xt{};
    HARD_ASSERT(
    (mutex->type & (from_enum(MutexType::Try) | from_enum(MutexType::Timed))) != 0, "Trylock not supported by Mutex"
    );
    xt.sec  = 0;
    xt.nsec = 0;
...
ThreadState __cdecl mutex_unlock(MutexHandle mutex) {
    HARD_ASSERT(
    1 <= mutex->count && mutex->thread_id == static_cast<long>(GetCurrentThreadId()), "Unlock of owned mutex"
    );
...
void* __cdecl mutex_getconcrtcs(MutexHandle mutex) {
    return mutex->_get_cs();
}
...
void __cdecl sharedmutex_lock_exclusive(SharedMutex* smutex) {
    AcquireSRWLockExclusive(cast<PSRWLOCK>(smutex));
}
void __cdecl sharedmutex_lock_shared(SharedMutex* smutex) {
    AcquireSRWLockShared(cast<PSRWLOCK>(smutex));
}
...
void __cdecl sharedmutex_unlock_exclusive(SharedMutex* smutex) {
    ReleaseSRWLockExclusive(cast<PSRWLOCK>(smutex));
}
void __cdecl sharedmutex_unlock_shared(SharedMutex* smutex) {
    ReleaseSRWLockShared(cast<PSRWLOCK>(smutex));
}
ThreadState __cdecl cond_init(CondHandle* condition) {
    *condition = nullptr;
    ...
    if (cond == nullptr) { return ThreadState::Nomem; }
