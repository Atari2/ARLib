...
class CircularListEntry {
    ...
    CircularListEntry(T&& entry, CircularListEntry<T>* prev, CircularListEntry<T>* next)
    requires MoveConstructible<T>
        : m_entry(move(entry)), m_prev(prev), m_next(next) {
        if (prev) prev->m_next = this;
        if (next) next->m_prev = this;
    }
    CircularListEntry(const T& entry, CircularListEntry<T>* prev, CircularListEntry<T>* next)
    ...
    CircularListEntry<T>& operator=(const CircularListEntry<T>& other)
    ...
    CircularListEntry<T>& operator=(CircularListEntry<T>&& other) noexcept
...
class CircularListIterator final : public CircularListIteratorBase<T> {
    ...
    CircularListIterator<T>& operator=(const CircularListIterator<T>& other) {
    ...
    CircularListIterator<T> operator-(int offset) {
        ...
        for (int i = 0; i < offset; i++) iter.internal_revert_();
        return iter;
    }
};
template <typename T>
...
class CircularDoublyLinkedList {
    ...
    CircularDoublyLinkedList(std::initializer_list<T> values) {
        for (auto v : values) internal_single_append_(Forward<T>(v));
    }
