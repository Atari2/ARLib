...
namespace ARLib {
    namespace Random {
        ...
        PCG PCG::create() {
            PCG state{0, 0};
            state.seed(static_cast<uint64_t>(Clock::now()) ^ reinterpret_cast<uintptr_t>(&ARLib::Clock::diff),
        ...
        void PCG::seed_s(uint64_t seed, uint64_t seq) { static_state().seed(seed, seq); }
        uint32_t PCG::random() {
        ...
        uint32_t PCG::random_s() { return static_state().random(); }
        ...
        uint32_t PCG::bounded_random_s(uint32_t bound) { return static_state().bounded_random(bound); }
    } // namespace Random
} // namespace ARLib
