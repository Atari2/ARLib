...
namespace ARLib {
	...
	class ListEntry {
		...
		ListEntry(T&& entry, ListEntry<T>* next) requires MoveConstructible<T>
			: m_entry(move(entry)), m_next(next) {

		}

		ListEntry(const T& entry, ListEntry<T>* next) requires CopyConstructible<T>
	...
	public:
		...
		bool operator==(const LinkedListIteratorBase<T>& other) const {
