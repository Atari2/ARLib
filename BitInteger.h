...
namespace ARLib {
    ...
    struct TypeArray {
        ...
        template <size_t I>
        requires(I < sizeof...(Types)) using At = RemoveCvRefT<decltype(declval<TupleType>().template get<I>())>;
    };
    ...
    class BitInteger {
        ...
        using T =
        ConditionalT<(Width > 8), ConditionalT<(Width > 16), ConditionalT<(Width > 32), uint64_t, uint32_t>, uint16_t>,
                     uint8_t>;
